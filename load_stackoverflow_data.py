...
if not client.collection_exists(environ["VECTOR_STORE_COLLECTION"]):
    ...
    client.create_collection(
        ...
        sparse_vectors_config={
            environ["SPARSE_VECTOR_NAME"]: models.SparseVectorParams()
        },
    )
...
with open(environ["QUESTIONS_PATH"], newline="", encoding="latin-1") as qfile:
    ...
    for row in reader:
        if (len(questions)) > int(environ["QUESTIONS_LIMIT"]):
            logging.info(
                f'Reached questions limit of {environ["QUESTIONS_LIMIT"]}. Stopping read.'
            )
            break
...
for qid, (question_title, question) in questions.items():
    ...
    dense_document = models.Document(
        text=text_content, model=environ["DENSE_MODEL_NAME"]
    )
    sparse_document = models.Document(
        text=text_content, model=environ["SPARSE_MODEL_NAME"]
    )
    documents.append(
        {
